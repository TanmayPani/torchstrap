...
def random_split(
    ...
    **kwargs,
    ...
    if num_replicas > 1:
        ...
        return [vmap(input.take_along_dim)(idx, dim=dim) for idx in index_splits for input in inputs]
    ...
    return [input.take_along_dim(idx, dim=dim) for idx in index_splits for input in inputs]
...
def undersample_and_random_split(
    ...
    *,
    ...
    if num_replicas > 1:
        vmap_in_dims = (vmap_in_dims, None) if isinstance(vmap_in_dims, int) else vmap_in_dims
        subset_idx = vmap(
            _undersample,
            in_dims = vmap_in_dims, 
            randomness=vmap_randomness,
        ...
        subset_label = vmap(label.__getitem__)(subset_idx) \
    ...
    subset_label_counts = [subset_label[subset_label == l].shape[0] \
                        for l in range(num_unique_label)] \
                        if subset_label is not None else []
...
def _stratified_split(
    ...
    *,
    ...
    label_indices = [
        sorted_one_hot_indices[:, l][:lcount]
        for l, lcount in enumerate(label_counts)
    ]
    ...
    return [
        split[
            torch.randperm(split.shape[0], generator=generator)
        ] for split in splits
    ] 
...
def undersample(
    ...
    **kwargs,
    ...
    if num_replicas > 1:
        ...
        return [torch.vmap(input.take_along_dim)(subsample_idx, dim=dim) for input in inputs]
...
def _undersample(
    ...
    *,
    ...
    if label is None or num_unique_label == 1:
        return input[
            torch.randperm(input_size, generator=generator)[:sub_size]
        ]
    ...
    label_sub_counts = [
        math.floor(float(sub_size/input_size)*lcount)
        for lcount in label_counts
    ]
    ...
    sub_index = torch.cat([
        lindex[
            torch.randperm(lindex.shape[0], generator=generator)[:lcount]] 
...
class MultiSubsetBatchSampler(TensorBatchSampler):
    def __init__(
        ...
        *,
        ...
        self.subsample_size = subsample_size if isinstance(subsample_size, int) \
...
def train_test_multi_subset_samplers(
    ...
    *,
    ...
    subsample_size = subsample_size if isinstance(subsample_size, int) \
                    else math.floor(length*subsample_size)

    train_size = train_size if isinstance(train_size, int) \
                else math.floor(subsample_size*train_size)

    if train_size > subsample_size:
        raise ValueError(
            "training set size must be smaller than subset size."
        )
...
if __name__ == "__main__":
    ...
    if do_stratify:
        stratify = torch.cat([torch.zeros(700),torch.ones(300),])
    else:
        stratify = None
    ...
    if stratify is not None:
        stratify_sub = vmap(stratify.__getitem__)(idx_sub) if num_replicas > 1 else stratify[idx_sub]
    ...
    if stratify is not None:
        stratify0 = vmap(stratify.__getitem__)(idx0) if num_replicas > 1 else stratify[idx0]
        stratify1 = vmap(stratify.__getitem__)(idx1) if num_replicas > 1 else stratify[idx1]
