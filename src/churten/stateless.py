...
class StatelessModule(Module):
    ...
    def infer(
        ...
        **kwargs,
        ...
        return vmap(
            ...
            state.param_dict, state.buffer_dict,
            *args, **kwargs,
        ).detach().cpu()
    ...
    def fit(
        ...
        *,
        valid_iterator : Optional[Iterator[tuple[Tensor, Tensor, Tensor]]] = None,
        num_epochs : int = 1,
        ...
        **kwargs,
        ...
        for input, target, sample_weight in iterator:
            ...
            batch_loss = self.step(
                ...
                criterion,
                state,
                input.to(device=state.device, dtype=state.dtype,),
                target.to(device=state.device, dtype=state.dtype,),
                sample_weight.to(device=state.device, dtype=state.dtype,) \
    ...
    def forward_eval(
        ...
        **kwargs,
        ...
        prediction = self._call_impl(
            params, buffers, X, **model_call_kwargs,
        )
        return criterion(prediction, y, weight=sample_weight, **kwargs)
